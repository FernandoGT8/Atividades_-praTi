    ...
    if (btnCadastro) {
        btnCadastro.addEventListener('click', async (e) => {
            ...
            try {
                ...
                if (btnContent) btnContent.textContent = 'Cadastrando...';
                btnCadastro.style.pointerEvents = 'none';

                // 2. REQUISIÇÃO ASSÍNCRONA PARA API PÚBLICA DE CADASTRO SIMULADO
                ...
                // SALVA O EMAIL LOCALMENTE PARA VALIDAÇÃO POSTERIOR
                // Usamos a propriedade vinda da API ou o valor do input (garantindo o fallback)
                const registradoEmail = data.email || emailValue;
                localStorage.setItem('laraflix_user_email', registradoEmail);

                // 3. ATUALIZAÇÃO DINÂMICA DO DOM (MENSAGEM DE SUCESSO)
                registrationForm.innerHTML = `
                    <div style="animation: fadeIn 0.5s ease; padding: 2rem; background: rgba(0,0,0,0.9); border-radius: 8px; border: 1px solid #e50914; text-align: center;">
                        <h3 style="color: #fff; font-size: 1.75rem; margin-bottom: 0.5rem;">Pronto! Conta criada.</h3>
                        <p style="color: #aaa; font-size: 1.1rem;">Usuário validado. Você já pode acessar o catálogo.<br><strong style="color: #fff;">${registradoEmail}</strong></p>
                        <p style="color: #aaa; font-size: 0.85rem; margin-top: 1.5rem;">ID registrado via API externa: <span style="color: #27ae60; font-family: monospace; font-weight: bold;">#${data.id || '999'}</span></p>
                    </div>
                `;
